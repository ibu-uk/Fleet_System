<?php
// ============================================================
//  driver-portal.php — Driver Mobile Portal Login
// ============================================================
session_start();
require_once __DIR__.'/config.php';
require_once __DIR__.'/functions.php';

// Already logged in → go to driver dashboard
if (!empty($_SESSION['user_id'])) {
    header('Location: index.php?module=driver_portal'); exit;
}
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (!$username || !$password) {
        ...
        $pdo  = getDB();
        $stmt = $pdo->prepare("SELECT * FROM users WHERE username=? AND status='active' AND role='driver' LIMIT 1");
        ...
        if ($user && password_verify($password, $user['password_hash'])) {
            ...
            $_SESSION['is_driver_portal'] = true;
...
<title><?= $isRTL ? 'بوابة السائقين — تسجيل الدخول' : 'Driver Portal — Login' ?></title>
...
.input-wrap input {
  ...
  border-radius: 9px;
  padding: 12px 12px 12px <?= $isRTL ? '12px' : '38px' ?>;
  <?= $isRTL ? 'padding-right: 38px;' : '' ?>
  color: #fff;
  font-size: 14px;
  font-family: <?= $isRTL ? "'Cairo'" : "'Syne'" ?>, sans-serif;
  outline: none;
...
<div class="login-card">
  <div class="login-brand">
    ...
    <div>
      <div class="login-brand-text"><?= $isRTL ? 'بوابة السائقين' : 'Driver Portal' ?></div>
  ...
  <form method="post" action="driver-portal.php" autocomplete="off">
  ...
  <div class="lang-row">
    <a href="driver-portal.php?lang=ar" class="lang-link <?= $lang==='ar'?'active':'' ?>">🇰🇼 عربي</a>
    <a href="driver-portal.php?lang=en" class="lang-link <?= $lang==='en'?'active':'' ?>">🇬🇧 English</a>
  </div>
</div>

<div class="login-footer">DRIVER PORTAL · v1.0 · Kuwait</div>
