...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if ($action === 'add' || $action === 'edit') {
        ...
        // Check if this is a free service based on vehicle-specific rules
        $isFree = isServiceFreeForVehicle($vid, $sKm, $sKm) ? 1 : 0;
        ...
        if (!$vid || !$fields['service_date'] || !$fields['service_type']) {
            ...
            try {
                if ($action === 'add') {
                    ...
                    $pdo->prepare("INSERT INTO vehicle_services ($cols) VALUES ($vals)")->execute(array_values($fields));
                    ...
                    updateVehicleServiceTracking($vid, $sKm, $svcNumber, (bool)$isFree);
                    // Notify driver if assigned
                    $vstmt = $pdo->prepare("SELECT current_driver_id FROM vehicles WHERE id=?");
                    $vstmt->execute([$vid]);
                    $veh = $vstmt->fetch();
                    if ($veh && $veh['current_driver_id']) {
                        $msg = "Service #{$svcNumber} completed for vehicle. Next service at {$nextKm}km";
                        if ($isFree) $msg .= " (This was a FREE service)";
                        createServiceNotification($vid, $veh['current_driver_id'], 'reminder', $msg, $pdo->lastInsertId(), $nextKm);
                    }
                    setFlash('success', t('record_saved'));
                } else {
                    $set=implode('=?,',array_keys($fields)).'=?';
                    $pdo->prepare("UPDATE vehicle_services SET $set WHERE id=?")->execute([...array_values($fields),$id]);
                    setFlash('success', t('record_updated'));
                }
            } catch (PDOException $e) { setFlash('danger', t('error_occurred').' '.$e->getMessage()); }
        }
        header("Location: ?module=services"); exit;
    }
    if ($action === 'delete') {
        $pdo->prepare("DELETE FROM vehicle_services WHERE id=?")->execute([$id]);
...
$baseWhere = " FROM vehicle_services vs
...
$sql="SELECT vs.*, v.plate_number, v.make, v.model, v.type AS vtype, v.current_km,
               v.service_counter, v.free_service_km_threshold, v.free_service_driver_id,
               v.km_since_last_service, v.free_service_counter,
               COALESCE(e.name_en,'—') AS driver_name,
               COALESCE(fs.name_en,'—') AS free_service_driver_name".$baseWhere;
...
<div class="card mb-3"><div class="card-body py-2">
  <form method="get" class="row g-2 align-items-end">
    ...
    <div class="col-auto"><a href="?module=services" class="btn btn-sm btn-outline-secondary"><?= t('reset') ?></a></div>
    <div class="col-auto ms-auto"><button type="button" class="btn btn-sm btn-success" onclick="openAddModal()"><i class="fas fa-plus me-1"></i><?= t('add_service') ?></button></div>
  </form>
</div></div>
...
<table class="table table-hover align-middle mb-0">
  <thead class="table-dark"><tr>
    ...
    <th><?= t('current_km') ?></th><th><?= t('service_counter') ?></th><th><?= t('cost') ?></th><th><?= t('garage_name') ?></th><th><?= t('actions') ?></th>
  </tr></thead>
  <tbody>
  ...
  <tr>
    ...
    <td>
      <span class="badge bg-info"><?= $r['service_counter'] ?></span>
      <?php if ($r['free_service_km_threshold']): ?>
        <small class="text-muted d-block">Free every <?= number_format($r['free_service_km_threshold']) ?>km</small>
        <small class="text-muted"><?= e($r['free_service_driver_name']) ?></small>
      <?php endif; ?>
    </td>
    <td>
      <?php if ($r['is_free_service']): ?>
        <span class="badge bg-success">FREE</span>
      <?php endif; ?>
      <?= $r['cost'] ? 'KWD '.number_format($r['cost'],3) : '—' ?>
    </td>
...
<nav class="mt-3"><ul class="pagination pagination-sm justify-content-center">
  ...
  <li class="page-item <?= $p===$page?'active':'' ?>"><a class="page-link" href="?module=services&page=<?= $p ?>&q=<?= e($search) ?>&stype=<?= e($fType) ?>&vtype=<?= e($fVehType) ?>&vehicle_id=<?= $fVeh ?>"><?= $p ?></a></li>
  <?php endfor; ?>
</ul></nav>
<?php endif; ?>
    ...
    <div class="modal-content">
      <form method="post" id="svcForm">
        ...
        <div class="modal-header bg-warning text-dark">
          <h5 class="modal-title" id="modalTitle"><?= t('add_service') ?></h5>
        ...
        <div class="modal-body">
          <div class="row g-3">
            ...
            <div class="col-md-3"><label class="form-label"><?= t('service_number') ?></label><input name="service_number" id="fSvcNo" type="number" class="form-control" value="1" min="1"><small class="text-muted" id="freeServiceHint"></small></div>
            <div class="col-md-4">
...
function loadVehicleKm(vehId){
  ...
  fetch('?module=api&action=get_vehicle_km&id='+vehId)
    ...
    .then(d=>{
      ...
      if(d.service_counter){
        document.getElementById('fSvcNo').value=d.service_counter+1;
      }
      // Show free service info if configured
      const hint=document.getElementById('freeServiceHint');
      if(d.free_service_km_threshold){
        hint.textContent='Free every '+d.free_service_km_threshold+'km for '+d.free_service_driver_name;
...
function openAddModal(){
  ...
  document.getElementById('modalTitle').textContent='<?= t('add_service') ?>';
...
function openEditModal(r){
  ...
  document.getElementById('modalTitle').textContent='<?= t('edit_service') ?>';
...
function confirmDelete(id){if(confirm('<?= t('confirm_delete') ?>')){const f=document.getElementById('deleteForm');f.action='?module=services&action=delete&id='+id;f.submit();}}
document.getElementById('svcForm').addEventListener('submit',function(){this.action='?module=services&action='+document.getElementById('fAction').value+(document.getElementById('fId').value?'&id='+document.getElementById('fId').value:'');});
